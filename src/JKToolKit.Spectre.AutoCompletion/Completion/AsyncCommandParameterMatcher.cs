    ...
    where T : CommandSettings
{
    ...
    /// Gets the suggestions for the specified parameter.
    /// </summary>
    /// <param name="parameterMap">Information on which parameter to get suggestions for.</param>
    /// <returns>The suggestions for the specified parameter.</returns>
    public async Task<CompletionResult> MatchAsync(IMappedCommandParameter parameterMap)
        => await MatchAsync(parameterMap.Parameter, parameterMap.Value ?? string.Empty);
